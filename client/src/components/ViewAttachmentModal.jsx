...
const ViewAttachmentModal = ({ attachment, onClose }) => {
	...
	const getFileIcon = (ext) => {
		if (/(jpg|jpeg|png|gif|bmp)/i.test(ext))
	...
	const renderContent = () => {
		if (loading)
			return (
				<div className="flex flex-col items-center justify-center h-full p-4">
					<div className="animate-spin h-12 w-12 border-b-2 border-pink-500 rounded-full mb-3" />
			...
			return (
				<div className="flex flex-col items-center justify-center h-full p-4">
					...
					<button
						...
						className="flex items-center space-x-2 bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded"
					>
						<Download />
						<span>Download</span>
					</button>
		...
		if (ext === "txt" || ext === "html") {
			return (
				<pre
					className={`bg-gray-100 rounded shadow-inner overflow-auto ${
		...
		if (ext === "zip") {
			return (
				<div className="overflow-auto p-2 max-h-[70vh] bg-white rounded shadow-inner">
		...
		return (
			<div className="flex flex-col items-center justify-center h-full p-8 text-center">
				...
				<button
					onClick={handleDownload}
					className="text-blue-600 underline hover:text-blue-800 cursor-pointer transition hover:underline"
				>
					Download instead
				</button>
			</div>
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			<div
				...
				} relative bg-gray-50 rounded-lg shadow-2xl overflow-hidden flex flex-col transition-all duration-300`}
			>
				<div className="flex items-center space-x-2 bg-white border-b px-4 py-2">
					{getFileIcon(ext)}
					<h3 className="flex-1 text-lg font-medium text-gray-800 truncate">
						{attachment.name}
					</h3>
					<div className="flex items-center space-x-1">
						...
						<button
							onClick={onClose}
							className="p-1 hover:bg-gray-200 rounded cursor-pointer hover:text-red-600"
							title="Close"
						>
							<X />
						</button>
